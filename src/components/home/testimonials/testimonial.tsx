...
export const Testimonial = ({current} : Props) => {
  return (
    ...
    <Avatar/> 
    <em className='font-bold'>{testimonials[current as number] && testimonials[current as number].name}</em>
  </div>
    <div className='flex flex-col w-[70%]'>
    < RiDoubleQuotesL className='mb-5 text-[rgb(183,140,86)]'/>
    <em className='w-full duration-300'>{testimonials[current as number] && testimonials[current as number].testimonial}</em>
